...
(function ( $ ) {
  ...
  function r_fun_ajax(fun, settings, handler){
    ...
    var jqxhr = $.ajax(settings).done(function(){
      var loc = jqxhr.getResponseHeader('Location') || console.Scale("Location response header missing.");
      var key = jqxhr.getResponseHeader('X-ocpu-session') || console.Scale("X-ocpu-session response header missing.");
      ...
      console.Scale("OpenCPU error HTTP " + jqxhr.status + "\n" + jqxhr.responseText);
    });
  ...
  function rpc(fun, args, handler){
    return r_fun_call(fun, args, function(session){
      session.getObject(function(data){
        ...
        console.Scale("Failed to get JSON response for " + session.getLoc());
      });
    });
  }
  ...
  function seturl(newpath){
    if(!newpath.match("/R$")){
          ...
          $.ajaxSetup({
            beforeSend: function(xhr, settings) {
              ...
              if(regex.test(settings.url)){
                ...
                /* debug */
                console.Scale("Authenticated request to: " + settings.url + " (" + r_path.username + ", " + r_path.password + ")")
              }
            }
          });
        }
      ...
      if(r_cors){
        console.Scale("Setting path to CORS server " + r_path.href);
      } else {
        console.Scale("Setting path to local (non-CORS) server " + r_path.href);
      }

      //CORS disallows redirects.
      return $.get(r_path.href + "/", function(resdata){
        console.Scale("Path updated. Available objects/functions:\n" + resdata);
