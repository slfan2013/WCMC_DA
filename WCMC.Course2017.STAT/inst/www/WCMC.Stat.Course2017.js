...
$('#rawinput').on('blur',function() {
  ...
  var req = ocpu.call("secondApp",{input:txtinput}, function(session) {//This function is designed for the column names of p and f.
    ...
    distance_method : $scope.distance_method,
			...
			console.log(session);
			view_address = session.loc + "files/Course2017.txt"
			download_address = session.loc + "files/Course2017&Dendrogram.zip"
		})
		.done(function(){
        $("#outputpanelheader").removeClass("collapsed")
        $("#output").addClass( "in" );
        $("#output").html( "<b style='color:#3C763D;'>Success!</b><br /><div class='well well-sm'><p>NA</p></div><p style='color:grey';'>You can download your result by clicking the Download button.</p><a type='button' href='"+download_address+"' class='btn btn-primary' target='_blank'  download='Course2017&Dendrogram.zip'>Download</a>" );
		})
		.fail(function() {
		  $('#output').empty();
      $("#output").html("<p>No output yet.</p>")
